...
struct MouseBinding {
  ...
  bool check_is_scrolled(GLFWwindow *window) {
    ...
    auto mods = (shift ? GLFW_MOD_SHIFT : 0) | (ctrl ? GLFW_MOD_CONTROL : 0) |
                (alt ? GLFW_MOD_ALT : 0);
    switch (modifier) {
      case Modifier::kShift:
        modPressed = mods == GLFW_MOD_SHIFT;
        break;
      case Modifier::kCtrl:
        modPressed = mods == GLFW_MOD_CONTROL;
        break;
      case Modifier::kAlt:
        modPressed = mods == GLFW_MOD_ALT;
        break;
      default:
        modPressed = mods == 0;
        break;
  ...
  bool check_is_pressed(GLFWwindow *window) {
    ...
    switch (mouseBtn) {
      case MouseButton::kLMB:
        button = GLFW_MOUSE_BUTTON_LEFT;
        break;
      case MouseButton::kMMB:
        button = GLFW_MOUSE_BUTTON_MIDDLE;
        break;
      case MouseButton::kRMB:
...
void cursor_pos_callback(GLFWwindow *window, double xpos, double ypos) {
  ...
  if (moving && inputPref.clamp_cursor &&
      (xpos >= width - 1 || ypos >= height - 1 || xpos <= 1 || ypos <= 1)) {
...
void mouse_button_callback(GLFWwindow *window, int button, int action,
  ...
  GLFWcursor *cursor = glfwCreateStandardCursor(moving ? GLFW_CROSSHAIR_CURSOR
                                                       : GLFW_ARROW_CURSOR);
