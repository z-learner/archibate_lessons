...
struct ObjWriter {
  ...
  static std::string int_to_string(size_t n) { return std::to_string(n); }
#endif
...
void mallocvis_plot_alloc_actions(std::vector<AllocAction> actions) {
    ...
    if (kAllocOpIsAllocation[(size_t)action.op]) {
      living.insert({action.ptr,
                     {action.op, action.op, action.tid, action.tid, action.ptr,
                      action.size, action.caller, action.caller, action.time,
                      action.time}});
    } else {
  ...
  if (options.height_scale == PlotOptions::Log) {
    ...
    eval_height = [](LifeBlock const &block) -> double { return block.size; };
  }
